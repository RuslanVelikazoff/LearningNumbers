...
namespace Plugins.Audio.Editor
{
    public class AudioManagementEditorWindow : EditorWindow
    {
        ...
        private void DrawMainScreen()
        {
            ...
            _audioConfiguration.CanDebug = GUILayout.Toggle(_audioConfiguration.CanDebug, "Debug");
            _audioConfiguration.CanValidate = GUILayout.Toggle(_audioConfiguration.CanValidate, "Validate");
            ...
            for (int index = 0; index < database.Items.Count; index++)
            {
                ...
                if (data.Clip != null)
                {
                    ...
                    {
                        DrawHelpBox("The clip is in the Resources folder, move it", MessageType.Error);
                    }
                    else if (extension != ".mp3")
                    {
                        DrawHelpBox("Recomendet use mp3 audio format", MessageType.Warning);
                    }
                }

                if (string.IsNullOrEmpty(data.Key))
                {
                    data.Key = Path.GetFileNameWithoutExtension(data.Name);
                    //EditorGUILayout.HelpBox("Please fill in the key field", MessageType.Error);
                }
                else if (data.Clip == null)
                {
                    DrawHelpBox("Please fill in the clip field", MessageType.Error);
                }
            ...
            DrawDropArea(listRECT);
        }

        private void DrawHelpBox(string message, MessageType type)
        {
            if (_audioConfiguration.CanValidate)
            {
                EditorGUILayout.HelpBox(message, type);
            }
        }

        private void DrawDropArea(Rect dropRect)
        {
            if (dropRect.Contains(Event.current.mousePosition))
            {
                ...
                else if (Event.current.type == EventType.DragPerform)
                {
                    ...
                    {
                        if (dragObject is AudioClip audioClip)
                        {
                            database.AddData("", audioClip);
                        }
                    }
            ...
            if (_isDropped)
            {
                GUI.Box(dropRect, "", EditorStyles.selectionRect);
            }
        }
        ...
        private void AddAllClips()
        {
            ...
            foreach (string assetPath in allPaths)
            {
                ...
                if (audioClip != null)
                {
                    database.AddData(Path.GetFileNameWithoutExtension(assetPath), audioClip);
                }
            }
